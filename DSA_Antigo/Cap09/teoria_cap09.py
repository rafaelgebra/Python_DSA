...
print(f"Será criado uma array de 2 dimenções usando o método função .diag(), o comando completo é arr22 = dsa.diag(dsa.arange(3)).")
...
print(f"Agora vamos criar um nosso array com 10 elementos.\nSerá feito pelo seguinte comando: arr23 = dsa.arange(10)")
arr23 = dsa.arange(10)
print(f"O resultado é:\n{arr23}\nLembrando a indexação começa por 0 em python.\n")
print(f"fazendo um novo fatiamento:\n{arr23[2:9:3]}\nEsse fatiamento é feito pelo comando arr23[2:9:3]\nO primeiro número é start (onde inicia a contagem no indece) o segundo é o end (é até onde vai a contagem dos indices) e o step (é os passos ou a quantidade de elementos vai pular).\n")
print(f"Criando dois novos arrays para fazer comparação entre eles.\nO primeiro é: a = dsa.array([1, 2, 3, 4]).\nO segundo é: b = dsa.array([4, 2, 2, 4]).\n")
print("Pode ser feito a comparação de duas formas, 1 por cada elemento individual, 2 uma comparação com a array completa.")
a = dsa.array([1, 2, 3, 4])
b = dsa.array([4, 2, 2, 4])
print(f"Para fazer a comparação por elemento/item a item é só usar o sinal de igualdade duas vezes. a == b.\nO resultado fica assim:\n {a == b}\nO resultado é em valor Booleano.")
print(f"Para fazer uma comparação GLOBAL é só usar a função arry.equal().\nCom essa função o resultado é esse:\n{dsa.array_equal(a, b)}\n")
print(f"Também da para achar o valor minimo e maximo. com os métodos .min() e .max().\nO valor minimo é: {arr23.min()}\nO valor máximo é: {arr23.max()}\n")
print(f"Também da para somar valor a cada elemento da array. Usando a criatividade. o código fica assim. dsa.array([1, 2, 3]) + 1.5. O resultado fica assim: {dsa.array([1, 2, 3]) + 1.5}\n")
print("Criando um array de tipo floot")
arr24 = dsa.array([1.2, 1.5, 1.6, 2.5,3.5, 4.5])
print(f"Os valores do array arr25, são:\n{arr24}\n")
print(f"Na biblioteca NumPy tem o método de arredondamento dos valores dentro da array, o método é .around()\nEm tão o comando fica arr25 = dsa.around(arr24).\n")
arr25 = dsa.around(arr24)
print(f"O resultado é:\n{arr25}\nO valor de arredondamento é o mesmo usado na matemática tanto para cima como para baixo.\n")
print("Criando um novo array, para usar com o metodo .flatten()")
arr26 = dsa.array([[1, 2, 3, 4], [5, 6, 7, 8]])
print(f"{arr26}\n")
print(f"O método .flatten() com NumPy é usado para criar uma cópia unidimensial (ou 'achatada') de uma array multidimencional. Isso significa que o método cria um novo array unidimensional, que contém todos os elementos do array multidimencional original, mas que está organizado em uma única linha. A ordem dos elementos no novo array unidimensional segue a ordem dos elementos no array multidimencional original.\n")
print("'Achatando' a matriz. Isso é feito assim. arr27 = arr26.flatten()")
arr27 = arr26.flatten()
print(arr27)
